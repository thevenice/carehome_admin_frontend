...
import { faFilter, faEye, faArrowAltCircleDown, faArrowAltCircleRight } from '@fortawesome/free-solid-svg-icons';
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-3 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-5">
          ...
          <div className="p-2.5 text-center xl:p-5">
            <h5 className="text-sm font-medium uppercase xsm:text-base">Phone Numbers</h5>
          </div>
          <div className="p-2.5 text-center xl:p-5">
            <h5 className="text-sm font-medium uppercase xsm:text-base">Staff Size</h5>
          </div>
          <div className="hidden p-2.5 text-center sm:block xl:p-5">
            <h5 className="text-sm font-medium uppercase xsm:text-base">Registration Date</h5>
          </div>
          <div className="hidden p-2.5 text-center sm:block xl:p-5">
        ...
        {careHomes.map((careHome:any, key) => (
          <div
            className={`grid grid-cols-3 sm:grid-cols-5 ${
              key === careHomes.length - 1 ? '' : 'border-b border-stroke dark:border-strokedark'
            }`}
            key={careHome._id}
          >
            <div className="flex items-center gap-3 p-2.5 xl:p-5">
              ...
              <p className="hidden text-black dark:text-white sm:block">{careHome.name}</p>
            </div>
            
            <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
            {careHome.contact_info.phone_number.map((item:any, index:number) => (
              <p className="text-black dark:text-white">{index}. {item}</p>
            )
            ...
            <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
              <p className="text-black dark:text-white">{careHome.staff_size}</p>
            </div>
        ...
        <div className="flex justify-center mt-4 space-x-2">
          ...
          {pagination.current_page > 1 && (
            <button
              onClick={() => setPagination((prev) => ({ ...prev, current_page: pagination.current_page - 1 }))}
