...
const CarePlanTable = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setPagination({
          ...
          totalCarePlans: response.data.total,
          currentPage: response.data.currentPage,
        })
      } else {
        console.error('Failed to fetch care plans')
      }
    } catch (error) {
  ...
  const handleCreateClick = () => {
    navigate(`/care-plans/create`)
  }

  const handleViewCarePlan = (carePlanId: any) => {
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-max table-auto">
          ...
          <tbody>
            {carePlans.map((carePlan:any, key) => (
              <tr
                key={carePlan._id}
                className={`${
                  key === carePlans.length - 1
                    ? ''
                    : 'border-b border-stroke dark:border-strokedark'
                }`}
              >
                <td className="py-4 px-4">
                  <p className="text-black dark:text-white text-sm truncate max-w-xs">
                    {carePlan.name}
                  </p>
                </td>
                <td className="py-4 px-4 text-center">
                  <p className="text-meta-5 text-sm">{carePlan.level}</p>
                </td>
                <td className="py-4 px-4 text-center">
                ...
                <td className="py-4 px-4 text-center">
                  <p className="text-black dark:text-white text-sm">
                    {carePlan.accommodationType}
                  </p>
                </td>
                <td className="py-4 px-4 text-center">
                  <button
                    onClick={() => handleViewCarePlan(carePlan._id)}
