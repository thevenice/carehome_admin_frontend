...
const CaregiverCreateUpdateForm: React.FC = () => {
  const { userId } = useParams<{ userId: string }>()
  const [formData, setFormData] = useState<CaregiverInfo>({
    contactNumber: '',
    address: '',
    specialization: '',
    licenseNumber: '',
  ...
  useEffect(() => {
    // Fetch caregiver info and set formData
    fetchCaregiverInfoData()
  }, [])

  const fetchCaregiverInfoData = async () => {
    try {
      const response = await axiosInstance.get(`/admin/caregivers?userId=${userId}`)
  ...
  const handleAddArrayField = (fieldName: keyof CaregiverInfo) => {
    setFormData((prevState) => ({
      ...prevState,
  ...
  const handleRemoveArrayField = (fieldName: keyof CaregiverInfo, index: number) => {
    setFormData((prevState) => ({
      ...prevState,
      [fieldName]: (prevState[fieldName as keyof CaregiverInfo] as string[]).filter((_, i) => i !== index),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('Caregiver info updated:', response.data)
    } catch (error) {
      console.error('Error updating caregiver info:', error)
...
const handleNestedRemoveArrayField = (parentKey: string | number, key: string | number, index: any) => {
  ...
  return (
    <>
      {formData ? (
        <DefaultLayout>
          <Breadcrumb pageName="Update Caregiver Info" />
          <div className="grid">
